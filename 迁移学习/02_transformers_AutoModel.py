"""
演示
    huggingface 的 transformers库 如何使用AutoModel自动模型方法来实现常见的NLP任务，
...
需要掌握的:
    AutoModel:
        # 1.tokenizer编码，获取输入文本的索引序列，文本 -> 分词 -> token序列 -> 索引序列
        inputs = tokenizer(
            **kwargs,   # 其他参数，比如text
...
# 导包
import torch
# 导入transformers库中的自动模型和分词器相关类
from transformers import (
    AutoConfig,  # 自动配置
    AutoTokenizer,  # 自动分词器
    AutoModel,  # 自动模型
    AutoModelForSequenceClassification,  # 自动模型，序列分类
    AutoModelForMaskedLM,  # 自动模型，掩码语言模型，用于 完形填空
    AutoModelForQuestionAnswering,  # 自动模型，问答/阅读理解
    AutoModelForSeq2SeqLM,  # 自动模型，序列到序列语言模型，用于 文本摘要/文本生成
    AutoModelForTokenClassification  # 自动模型，命名实体识别NER/词性标注POS
)

# 1.定义通用函数，实现transformers的AutoModel方法 的推理过程，模拟上线效果
@torch.no_grad() # 关闭梯度计算
def demo_automodel(model, tokenizer, **kwargs):
    """
    这是一个通用的推理函数，支持各种 automodel, 自动处理 输入文本/数据，返回结果
...
if __name__ == '__main__':
    ...
    # 3.模型推理
    outputs, inputs = demo_automodel(my_model, my_tokenizer, text=text)
    ...
    # # 1.加载模型和分词器tokenizer
    # my_model = AutoModelForMaskedLM.from_pretrained(
    ...
    #
    # # 2.定义输入数据text
    # text = "我叫阳帆，在中山大学读博士，毕业以后留在了食堂当门卫，现在有个对象，是新东方的厨师"
    # # 3.模型推理，得到output,里面有logits
    # outputs, inputs = demo_automodel(my_model, my_tokenizer, text=text)
    ...
    # 1.加载模型和分词器tokenizer
    my_model = AutoModelForQuestionAnswering.from_pretrained(
    ...
    # 2.定义输入数据
    context = "《三体2: 黑暗森林》是刘慈欣的科幻小说，是三体系列的第2部。主人公有唐鹏杰、伟雄、志斌、晋安、罗辑、三体人、伟杰、湘皖"
    questions = [
        "三体2的作者是谁？",
        "三体2的主人公有哪些？",
    ]
